...
`./`               | `pricecoinx.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `litecoind` or `pricecoinx-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `litecoind.pid`        | Stores the process ID (PID) of `litecoind` or `pricecoinx-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `litecoind` or `pricecoinx-qt`; can be specified by `-debuglogfile` option
