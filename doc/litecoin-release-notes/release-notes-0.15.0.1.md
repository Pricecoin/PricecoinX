...
or `litecoind`/`pricecoinx-qt` (on Linux).
...
Litecoin Core 0.15.0 contains the following changes to the RPC interface and `pricecoinx-cli` for multi-wallet:

* When running Litecoin Core with a single wallet, there are **no** changes to the RPC interface or `pricecoinx-cli`. All RPC calls and `pricecoinx-cli` commands continue to work as before.
