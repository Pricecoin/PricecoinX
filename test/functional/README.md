...
with the pricecoinxd(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a pricecoinxd.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the PricecoinX Core node application logic. For custom behaviour, subclass the
...
Test objects for interacting with a pricecoinxd node over the p2p interface.
