...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " pricecoinx-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  pricecoinx-tx [options] <hex-tx> [commands]  Update hex-encoded pricecoinx transaction\n" +
            "or:     pricecoinx-tx [options] -create [commands]   Create hex-encoded pricecoinx transaction\n" +
            "\n";
