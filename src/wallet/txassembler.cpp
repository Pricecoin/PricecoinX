    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional PRCX outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes PRCX and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::PRCX_TO_PRCX) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        if (SelectCoins(new_tx, nTargetValue, params_pegin)) {
            return std::any_of(new_tx.selected_coins.cbegin(), new_tx.selected_coins.cend(), is_prcx);
        }
    } else {
        // First try to construct a PRCX-to-PRCX transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::PRCX_ONLY;
        ...
        // If PRCX-to-PRCX fails, create a peg-out transaction
